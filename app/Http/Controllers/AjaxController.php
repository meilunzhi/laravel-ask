...
class AjaxController extends Controller
{
    ...
    {
        $total    = Notification::where('to_user_id', '=', Auth()->user()->id)
                                ->where('is_read', '=', 0)
                                ->count();
    ...
    {
        $total    = Message::where('to_user_id', '=', Auth()->user()->id)
                           ->where('is_read', '=', 0)
                           ->where("to_deleted", "<>", 1)
    ...
    public function loadInviteUsers(Request $request)
    {
        ...
        if (trim($word)) {
            ...
            $users->map(function ($user) use ($tagIds, $question){
                $user->tag_name    = '';
                $user->tag_answers = 0;
                $userTag           = UserTag::where("user_id", "=", $user->id)
            ...
            $invitations    = $question->invitations()
                                       ->get();
            $invitedUserIds = array_pluck($invitations, 'user_id');
            $userTags       = UserTag::whereIn("tag_id", $tagIds)
                                     ->whereNotIn("user_id", $invitedUserIds)
                                     ->orderBy("answers", "desc")
                                     ->orderBy("supports", "desc")
                                     ->select("user_id", "tag_id", "answers", "supports")
                                     ->take(16)
                                     ->groupBy("user_id")
                                     ->get();
            $users          = [];
    ...
    public function sendSmsCode(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($sendType == 'register' && User::where("mobile", "=", $mobile)
                                               ->count() > 0) { //注册发送处理
                return $this->ajaxError(10011, '该手机号已注册，不能重复注册');
            } else {
                if ($sendType == 'findPassword' && User::where("mobile", "=", $mobile)
                                                       ->count() == 0) { //找回密码处理
