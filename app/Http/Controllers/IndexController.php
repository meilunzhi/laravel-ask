...
class IndexController extends Controller
{
    ...
    public function index()
    {
        ...
        $recommendItems = Cache::remember('recommend_items', Setting()->get('website_cache_time', 1), function (){
            return Recommendation::where('status', '>', 0)
                                 ->orderBy('sort', 'asc')
        ...
        $friendshipLinks = Cache::remember('friendship_links', Setting()->get('website_cache_time', 1), function (){
            return FriendshipLink::where('status', '=', 1)
                                 ->orderBy('sort', 'asc')
        ...
        return view('theme::home.index')->with(compact('hotUsers', 'recommendItems', 'hotExperts', 'newestQuestions', 'rewardQuestions', 'hotArticles', 'newestArticles', 'newestNotices', 'hotTags',
            'topCoinUsers', 'friendshipLinks', 'signDate'));
    }
    ...
    public function ask($categorySlug = 'all', $filter = 'newest')
    {
        ...
        return view('theme::home.ask')->with(compact('currentCategory', 'questions', 'hotUsers', 'hotTags', 'filter', 'categories', 'currentCategoryId', 'parentId', 'parentCategories',
            'categorySlug'));
    }
    ...
    public function blog($categorySlug = 'all', $filter = 'newest')
    {
        ...
        return view('theme::home.blog')->with(compact('tabData', 'currentCategory', 'articles', 'hotUsers', 'hotTags', 'filter', 'currentCategoryId', 'parentId', 'parentCategories', 'categorySlug',
            'hotArticles'));
    }
    ...
    public function topic($categorySlug = 'all')
    {
        ...
        $parentCategories = Category::getParentCategories($parentId);
        $topics           = $query->orderBy('followers', 'DESC')
                                  ->paginate(20);
    ...
    public function experts(Request $request, $categorySlug = 'all', $provinceId = 'all')
    {
        ...
        $hotProvinces = Cache::remember('hot_expert_cities', Setting()->get('website_cache_time', 1), function (){
            return Authentication::select('province', DB::raw('COUNT(user_id) as total'))
                                 ->groupBy('province')
                                 ->orderBy('total', 'desc')
                                 ->get();
        });
        $query        = Authentication::leftJoin('user_data', 'user_data.user_id', '=', 'authentications.user_id')
        ...
        $experts = $query->orderBy('user_data.answers', 'DESC')
                         ...
                         ->select('authentications.user_id', 'authentications.real_name', 'authentications.description', 'authentications.title', 'user_data.coins', 'user_data.credits',
                             'user_data.followers', 'user_data.supports', 'user_data.answers', 'user_data.articles', 'user_data.authentication_status')
                         ->paginate(16);
