...
class AttachController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        if ($validator->fails()) {
            return $this->ajaxError(10000, $validator->errors()
                                                     ->first('file'));
        }
        ...
        Storage::disk('local')
               ->put($filePath, File::get($file));

        return $this->ajaxSuccess([
            'url' => route("website.attach.download", ['name' => str_replace("/", "-", $filePath)]), 'name' => $file->getClientOriginalName(),
        ]);
    }
