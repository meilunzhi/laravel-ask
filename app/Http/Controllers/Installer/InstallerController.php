...
class InstallerController extends Controller
{
    ...
    public function config(Request $request)
    {
        if ($request->isMethod('post')) {
            $validateRules = [
                'database_host' => 'required|max:128', 'database_port' => 'required|digits_between:0,65535', 'database_username' => 'required|max:128', 'database_password' => 'sometimes|max:128',
                'database_name' => 'required|max:128', 'database_prefix' => 'required|max:64',
            ];
            $request->flash();
            $this->validate($request, $validateRules);
            $envData = [
                'APP_NAME'    => str_random(12), 'APP_ENV' => 'local', 'APP_DEBUG' => 'false', 'APP_KEY' => str_random(32), 'DB_CONNECTION' => $request->input('database_driver'),
                'DB_HOST'     => $request->input('database_host'), 'DB_PORT' => $request->input('database_port'), 'DB_DATABASE' => $request->input('database_name'),
                'DB_USERNAME' => $request->input('database_username'), 'DB_PASSWORD' => $request->input('database_password'), 'DB_PREFIX' => $request->input('database_prefix'),
            ];
    ...
    public function website(Request $request, UserRepository $userRepository)
    {
        if ($request->isMethod('post')) {
            $validateRules = [
                'website_name'       => 'required|max:256', 'website_url' => 'required|max:128', 'website_admin_name' => 'required|max:128', 'website_admin_email' => 'required|email',
            ...
            $registerData = [
                'name'     => $request->input('website_admin_name'), 'email' => $request->input('website_admin_email'), 'password' => $request->input('website_admin_pass'), 'status' => 1,
