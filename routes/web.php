...
/*installer*/
Route::Group(
    ['namespace' => 'Installer', 'middleware' => 'installer'],
    function (){
...
/*问答*/
Route::get('/questions/{category_name?}/{filter?}', ['as' => 'website.ask', 'uses' => 'IndexController@ask'])
     ->where(['filter' => '(newest|hottest|reward|unAnswered)']);

/*标签*/
Route::get('/topics/{category_name?}', ['as' => 'website.topic', 'uses' => 'IndexController@topic']);

/*文章*/
Route::get('/articles/{category_name?}/{filter?}', ['as' => 'website.blog', 'uses' => 'IndexController@blog'])
     ->where(['filter' => '(recommended|newest|hottest)']);
...
Route::Group(
    ...
    function (){
        ...
        Route::get('oauth/{type}/login', ['as' => 'auth.oauth.login', 'uses' => 'OauthController@login'])
             ->where(['type' => '(qq|weibo|weixinweb|weixin)']);
        ...
        /*用户空间首页*/
        Route::get('people/{user_id}', ['as' => 'auth.space.index', 'uses' => 'SpaceController@index'])
             ->where(['user_id' => '[0-9]+']);
        /*我的提问*/
        Route::get('people/{user_id}/questions', ['as' => 'auth.space.questions', 'uses' => 'SpaceController@questions'])
             ->where(['user_id' => '[0-9]+']);
        /*我的回答*/
        Route::get('people/{user_id}/answers', ['as' => 'auth.space.answers', 'uses' => 'SpaceController@answers'])
             ->where(['user_id' => '[0-9]+']);
        /*我的文章*/
        Route::get('people/{user_id}/articles', ['as' => 'auth.space.articles', 'uses' => 'SpaceController@articles'])
             ->where(['user_id' => '[0-9]+']);
        /*我的讲座*/
        Route::get('people/{user_id}/courses/{filter?}', ['as' => 'auth.space.courses', 'uses' => 'SpaceController@courses'])
             ->where(['user_id' => '[0-9]+', 'filter' => '(involved|started)']);

        /*我的粉丝*/
        Route::get('people/{user_id}/followers', ['as' => 'auth.space.followers', 'uses' => 'SpaceController@followers'])
             ->where(['user_id' => '[0-9]+']);
        /*我的关注*/
        Route::get('people/{user_id}/followed/{source_type}', ['as' => 'auth.space.attentions', 'uses' => 'SpaceController@attentions'])
             ->where(['user_id' => '[0-9]+', 'source_type' => '(questions|tags|users)']);
        /*我的收藏*/
        Route::get('people/{user_id}/collected/{source_type}', ['as' => 'auth.space.collections', 'uses' => 'SpaceController@collections'])
             ->where(['user_id' => '[0-9]+', 'source_type' => '(questions|articles|courses)']);

        /*我的金币*/
        Route::get('people/{user_id}/coins', ['as' => 'auth.space.coins', 'uses' => 'SpaceController@coins'])
             ->where(['user_id' => '[0-9]+']);
        /*我的经验*/
        Route::get('people/{user_id}/credits', ['as' => 'auth.space.credits', 'uses' => 'SpaceController@credits'])
             ->where(['user_id' => '[0-9]+']);
        ...
        Route::Group(
            ['middleware' => 'auth'],
            function (){
                Route::get('doings/{filter?}', ['as' => 'auth.doing.index', 'uses' => 'DoingsController@index'])
                     ->where(['filter' => '(newest|concerned)']);
                ...
                Route::post('/user/drafts/create/{type}', ['as' => 'auth.draft.create', 'uses' => 'DraftController@create'])
                     ->where(['type' => '(question|article|answer)']);
                /*用户举报*/
        ...
        Route::any('search/{filter?}', ['as' => 'auth.search.index', 'uses' => 'SearchController@index'])
             ->where(['filter' => '(all|questions|articles|tags|users|courses)']);

        /*邮箱token验证*/
        Route::get('email/{action}/{token}', ['as' => 'auth.email.verifyToken', 'uses' => 'EmailController@verifyToken'])
             ->where(['action' => '(register|verify)']);
        ...
        Route::Group(
            ...
            function (){
                ...
                /*用户个人信息修改*/
                Route::any('profile/anyBase', ['uses' => 'ProfileController@anyBase'])
                     ->name('auth.profile.base');
                Route::post('profile/postAvatar', ['uses' => 'ProfileController@postAvatar'])
                     ->name('auth.profile.avatar');
                Route::any('profile/anyPassword', ['uses' => 'ProfileController@anyPassword'])
                     ->name('auth.profile.password');
                Route::any('profile/anyEmail', ['uses' => 'ProfileController@anyEmail'])
                     ->name('auth.profile.email');
                Route::any('profile/anyMobile', ['uses' => 'ProfileController@anyMobile'])
                     ->name('auth.profile.mobile');
                Route::any('profile/anyOauth', ['uses' => 'ProfileController@anyOauth'])
                     ->name('auth.profile.oauth');
                Route::any('profile/anyNotification', ['uses' => 'ProfileController@anyNotification'])
                     ->name('auth.profile.notification');
                Route::post('/profile/account', ['uses' => 'ProfileController@account'])
                     ->name('auth.profile.account');


                /*行家认证*/
                Route::get('authentication/getIndex', ['uses' => 'AuthenticationController@getIndex'])
                     ->name('auth.authentication.index');
                Route::any('authentication/anyEdit', ['uses' => 'AuthenticationController@anyEdit'])
                     ->name('auth.authentication.edit');
                Route::post('authentication/postStore', ['uses' => 'AuthenticationController@postStore'])
                     ->name('auth.authentication.store');

                /*我的通知*/
                Route::get('notifications/getIndex', ['uses' => 'NotificationController@getIndex'])
                     ->name('auth.notification.index');
                Route::get('notifications/getReadAll', ['uses' => 'NotificationController@getReadAll'])
                     ->name('auth.notification.readAll');

                /*我的私信*/
                Route::get('messages', ['as' => 'auth.message.index', 'uses' => 'MessageController@index']);
                Route::get('message/{user_id}', ['as' => 'auth.message.show', 'uses' => 'MessageController@show'])
                     ->where(['user_id' => '[0-9]+']);
                Route::get('message/destroy/{id}', ['as' => 'auth.message.destroy', 'uses' => 'MessageController@destroy'])
                     ->where(['id' => '[0-9]+']);
                Route::get('message/destroySession/{id}', ['as' => 'auth.message.destroySession', 'uses' => 'MessageController@destroySession'])
                     ->where(['from_user_id' => '[0-9]+']);
                ...
                Route::get('collect/{source_type}/{source_id}', ['as' => 'auth.collection.store', 'uses' => 'CollectionController@store'])
                     ->where(['source_type' => '(question|article|course)', 'source_id' => '[0-9]+']);

                /*关注问题、人、标签*/
                Route::get('follow/{source_type}/{source_id}', ['as' => 'auth.attention.store', 'uses' => 'AttentionController@store'])
                     ->where(['source_type' => '(question|tag|user)', 'source_id' => '[0-9]+']);
            }
        );

        /*点赞*/
        Route::get('support/{source_type}/{source_id}', ['as' => 'auth.support.store', 'uses' => 'SupportController@store'])
             ->where(['source_type' => '(answer|article|comment)', 'source_id' => '[0-9]+']);
        Route::get('support/check/{source_type}/{source_id}', ['as' => 'auth.support.check', 'uses' => 'SupportController@check'])
             ->where(['source_type' => '(answer|article|comment)', 'source_id' => '[0-9]+']);
    }
);
...
Route::Group(
    ...
    function (){
        /*问题查看*/
        Route::get('question/{id}', ['as' => 'ask.question.detail', 'uses' => 'QuestionController@detail'])
             ->where(['id' => '[0-9]+']);

        /*回答详情查看*/
        Route::get('question/{question_id}/answer/{id}', ['as' => 'ask.answer.detail', 'uses' => 'AnswerController@detail'])
             ->where(['id' => '[0-9]+', 'question_id' => '[0-9]+']);
        ...
        Route::Group(
            ...
            function (){
                ...
                /*问题修改*/
                Route::get('question/edit/{id}', ['as' => 'ask.question.edit', 'uses' => 'QuestionController@edit'])
                     ->where(['id' => '[0-9]+']);
                Route::post('question/update', ['middleware' => 'ban.user', 'as' => 'ask.question.update', 'uses' => 'QuestionController@update']);

                /*追加悬赏*/
                Route::post('question/{id}/appendReward', ['as' => 'ask.question.appendReward', 'uses' => 'QuestionController@appendReward'])
                     ->where(['id' => '[0-9]+']);

                /*邀请回答*/
                Route::get('question/invite/{question_id}/{to_user_id}', ['as' => 'ask.question.invite', 'uses' => 'QuestionController@invite'])
                     ->where(['question_id' => '[0-9]+', 'to_user_id' => '[0-9]+']);
                Route::any('question/inviteEmail/{question_id}', ['as' => 'ask.question.inviteEmail', 'uses' => 'QuestionController@inviteEmail'])
                     ->where(['question_id' => '[0-9]+']);
                Route::get('question/{question_id}/invitations/{type}', ['as' => 'ask.question.invitations', 'uses' => 'QuestionController@invitations'])
                     ->where(['question_id' => '[0-9]+', 'type' => '(part|all)']);

                /*采纳回答*/
                Route::get('answer/adopt/{id}', ['as' => 'ask.answer.adopt', 'uses' => 'AnswerController@adopt'])
                     ->where(['id' => '[0-9]+']);

                /*回答保存*/
                Route::post('answer/store', ['as' => 'ask.answer.store', 'uses' => 'AnswerController@store']);
                /*回答编辑页面显示*/
                Route::get('answer/edit/{id}', ['as' => 'ask.answer.edit', 'uses' => 'AnswerController@edit'])
                     ->where(['id' => '[0-9]+']);
                /*回答保存*/
                Route::post('answer/update/{id}', ['as' => 'ask.answer.update', 'uses' => 'AnswerController@update'])
                     ->where(['id' => '[0-9]+']);
        ...
        /*标签首页*/
        Route::get('topic/{id}/{source_type?}', ['as' => 'ask.tag.index', 'uses' => 'TagController@index'])
             ->where(['id' => '[0-9]+', 'source_type' => '(questions|articles|courses|details)']);

        /*加载评论*/
        Route::get('{source_type}/{source_id}/comments', ['as' => 'ask.comment.show', 'uses' => 'CommentController@show'])
             ->where(['source_type' => '(question|answer|article|video)', 'source_id' => '[0-9]+']);
    }
);
...
Route::Group(
    ...
    function (){
        /*文章查看*/
        Route::get('article/{id}', ['as' => 'blog.article.detail', 'uses' => 'ArticleController@show'])
             ->where(['id' => '[0-9]+']);
        ...
        Route::Group(
            ...
            function (){
                ...
                Route::get('article/edit/{id}', ['as' => 'blog.article.edit', 'uses' => 'ArticleController@edit'])
                     ->where(['id' => '[0-9]+']);
...
Route::Group(
    ...
    function (){
        /*商品详情查看*/
        Route::get('goods/{id}', ['as' => 'shop.goods.detail', 'uses' => 'GoodsController@show'])
             ->where(['id' => '[0-9]+']);
...
/*后台管理部分处理*/
Route::Group(
    ['prefix' => 'admin', 'namespace' => 'Admin', 'middleware' => ['auth', 'auth.admin', 'operation.log', 'ban.ip']],
    function (){
        /*用户登陆*/
        ...
        /*商品兑换*/
        Route::get('exchange/{id}/{status}', ['as' => 'admin.exchange.changeStatus', 'uses' => 'ExchangeController@changeStatus'])
             ->where(['id' => '[0-9]+', 'status' => '(success|failed)']);
...
/*加载省份城市信息*/
Route::get('ajax/loadCities/{province_id}', ['as' => 'website.ajax.loadCities', 'uses' => 'AjaxController@loadCities'])
...
Route::get('image/avatar/{avatar_name}', ['as' => 'website.image.avatar', 'uses' => 'ImageController@avatar'])
